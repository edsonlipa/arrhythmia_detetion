    ...
    for d in data_names:
        ...
        for i in range(beat_len):
            if sym[i] in labels and pos[i]-widb>=0 and pos[i]+wida+1<=sig_len:
                ...
                y_a = np.arange(pos[i]-widb,pos[i]+wida+1)
                print('ya:',y_a)                
                print('a:',a)
                b = sig[pos[i]-100:pos[i]+100+1]
                print('yb:',y_a)                
                y_b = np.arange(pos[i]-100,pos[i]+100+1)
                print('b:',b)
                ...
                plt.plot(y_a,a,linewidth=5)
                plt.plot(y_b,b,linewidth=5)
                plt.title=('comparacion de ventanas')
                plt.legend(['360','99:160','100:100'])
                plt.show()
        #print('X : ',X)
        #print('Y : ',Y)
